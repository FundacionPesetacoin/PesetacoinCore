...
*Setup instructions for a gitian build of Pesetacoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pesetacoin
...
- [Building Pesetacoin](#building-pesetacoin)
...
Clone the git repositories for pesetacoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/pesetacoin/pesetacoin
```
...
In [doc/release-process.md](release-process.md) in the pesetacoin repository under 'Fetch and build inputs'.
...
To build Pesetacoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the pesetacoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pesetacoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/pesetacoin/pesetacoin
...
For example:
```bash
URL=https://github.com/langerhans/pesetacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pesetacoin=${COMMIT} --url pesetacoin=${URL} ../pesetacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pesetacoin=${COMMIT} --url pesetacoin=${URL} ../pesetacoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/pesetacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pesetacoin-build.assert
```
...
[pesetacoin/gitian.sigs](https://github.com/pesetacoin/gitian.sigs/) repository, or
