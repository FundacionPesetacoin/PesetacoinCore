...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (auxpow != NULL) {
        ...
        // print to log
        LogPrintf("PesetacoinMiner: AUX proof-of-work found; our hash: %s ; parent hash: %s ; target: %s\n",
               hash.GetHex().c_str(),
    ...
    else
    {
        ...
        // print to log
        LogPrintf("PesetacoinMiner: proof-of-work found; hash: %s ; target: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    }
...
void static PesetacoinMiner(CWallet *pwallet)
{
    LogPrintf("PesetacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pesetacoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running PesetacoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("PesetacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PesetacoinMiner, pwallet));
}
