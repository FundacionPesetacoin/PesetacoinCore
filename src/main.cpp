...
#if defined(NDEBUG)
# error "Pesetacoin cannot be compiled without assertions."
#endif
...
//TODO: this isn't identical to pesetacoin reference client.
...
static const int64_t nTargetTimespan = 2 * 60 * 60; // Pesetacoin: every 2 hours
static const int64_t nTargetSpacing = 60; // Pesetacoin: 1 minute
...
{
        static const int64_t  BlocksTargetSpacing = 1 * 60; // 60 seconds PTC Block Target //CryptoMP
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        { 
                // CryptoMP - Kimoto Will Start at block 46.000 // FIXED
                if (pindexLast->nHeight+1 >= 46000) 
                	DiffMode = 2;
                if (pindexLast->nHeight+1 >= 582500)
                	DiffMode = 3;
...
void ThreadScriptCheck() {
    RenameThread("pesetacoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Pesetacoin
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
	...
	if (nHeight < 555000)
	{
		...
		if (block.nBits != GetNextWorkRequired(pindexPrev, &block))
			LogPrintf("AcceptBlock() : incorrect proof of work nBits: %d : GetNextWorkRequired: %d ",block.nBits, GetNextWorkRequired(pindexPrev, &block));		
	}
	else
	{
